...
Status: PRD approved with amendments — Gate 1 passed. **T1–T16 built — Wave 0 is complete, so the block can start.** Wave 1's chain is complete: T18–T21 built, T17 deferred within the wave. **Wave 2 is complete: T22–T25 built.** T26–T28 remain as a prioritized backlog (v1.8), specced and built one at a time in wave order. See the Amendments log at the end of this file for what changed from v1.0 and why.
